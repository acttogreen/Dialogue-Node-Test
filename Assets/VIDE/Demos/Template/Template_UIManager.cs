...
public class Template_UIManager : MonoBehaviour
{
    ...
    private VIDE_Assign vide_Assigned;

    void Awake()
    {
        vide_Assigned = GetComponent<VIDE_Assign>();
    }
    ...
    public void Interaction()
    {
        ...
        if (!VD.isActive) Begin(vide_Assigned);
        else CallNext();
    }
    ...
    public virtual void Begin(VIDE_Assign dialogue)
    {
        ...
        //VD.BeginDialogue(dialogToLoad); //Begins dialogue, will call the first OnNodeChange
    ...
    void UpdateUI(VD.NodeData data)
    {
        ...
        if (data.isPlayer)
        {
            ...
            if (data.tag.Length > 0) playerLabel.text = data.tag;
            else if (!string.IsNullOrEmpty(VD.assigned.alias)) playerLabel.text = VD.assigned.alias;
